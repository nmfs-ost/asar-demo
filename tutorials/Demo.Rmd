...
`asar` offers a variety of customization features including the option to select a subset of sections using the argument `custom=TRUE` and `custom_sections` or add a new sections within the default (or custom selection) using the arguments `new_section` and `section_location`. 

For example, if the user only wants to write a stock assessment report update that contains new data and the results. Run the following code:

```{r, echo=TRUE, eval=FALSE, stylelines=list(c(15), "highlight")}
asar::create_template(
  format = "pdf",
  office = "NWFSC",
  region = "U.S. West Coast",
...
You can also adjust figure captions and alternative text from the arguments `fig-cap` and `fig-alt`. These two areas contain pre-generated generic text that applies to the table/figure. To edit, simply make changes to the sentence(s) between the quotation marks.
